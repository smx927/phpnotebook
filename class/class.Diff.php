...
class Diff{
  ...
  public static function compare(
    ...
    if ($compareCharacters){
      ...
      $sequence1 = preg_split('/['.PHP_EOL.']/', $string1);  // '/\R/' [\r\n] PHP_EOL  
      $sequence2 = preg_split('/['.PHP_EOL.']/', $string2);
   ...
   //  while (count($partialDiff) > 0) $diff[] = array_pop($partialDiff);
    ...
    while (count($partialDiff) > 0){ 
    	...
    	$temp = array_slice($partialDiff, -1, 1); 
    	array_pop($partialDiff);
    	$diff[key($temp)] = reset($temp);
  ...
  private static function generatePartialDiff(
    ...
    while ($index1 > 0 || $index2 > 0){
      ...
      if ($index1 > 0 && $index2 > 0
        ...
        // update the diff and the indices
        $diff['_'.($index2 + $start)] = array($sequence2[$index2 + $start - 1], self::INSERTED);
        $index2 --;

      }else{

        // update the diff and the indices
        $diff[($index1 + $start).'_'] = array($sequence1[$index1 + $start - 1], self::DELETED); 
        $index1 --;
  ...
  public static function toString($diff, $separator = "\n"){
    ...
    foreach ($diff as $key => $line){
      ...
      switch ($line[1]){
        ...
        case self::DELETED    : 
        	$string_arr[trim($key,'_')][]=trim($key,'_').'行<font color=gray   > -&nbsp;'.htmlspecialchars($line[0]).'</font>';
        	// $string .= trim($key,'_').'行<font color=red   > -&nbsp;'.$line[0].'</font>'; 
        	// $string .= $separator; 
        	
        	break;
        case self::INSERTED   : 
        	$string_arr[trim($key,'_')][]=trim($key,'_').'行<font color=red >+&nbsp;'.htmlspecialchars($line[0]).'</font>';
        	// $string .= trim($key,'_').'行<font color=green >+&nbsp;'.$line[0].'</font>'; 
        	// $string .= $separator; 
    ...
    $re_string ='';
    foreach($string_arr as $kindex=>$val_arr){
       $re_string.= implode($separator,$val_arr).$separator;	
